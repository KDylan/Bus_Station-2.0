...
- (NSString *)ISO8601WeekDateStringWithTime:(BOOL)includeTime timeSeparator:(unichar)timeSep {
  ...
  unsigned yearIsLeapYear = is_leap_year(year);
  unsigned  prevYearIsLeapYear = is_leap_year(prevYear);
...
//  [gregorian release];
  ...
  return [NSString stringWithFormat:@"%lu-W%02u-%02u%@", (unsigned long)year, week, dayOfWeek + 1U, timeString];
}
- (NSString *)ISO8601OrdinalDateStringWithTime:(BOOL)includeTime timeSeparator:(unichar)timeSep {
  ...
  NSInteger dayOfYear = [gregorian ordinalityOfUnit: NSCalendarUnitDay inUnit: NSCalendarUnitYear forDate: self];
  NSString *timeString;
 ...
 //   [formatter release];
  ...
  return [NSString stringWithFormat:@"%ld-%03ld%@", (long)year, (long)dayOfYear, timeString];
}
