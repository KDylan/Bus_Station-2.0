...
+ (NSDate *)dateWithString:(NSString *)str strictly:(BOOL)strict timeSeparator:(unichar)timeSep getRange:(out NSRange *)outRange {
  ...
  if(strict && isspace(*ch)) {
    ...
    unsigned i = 0U;
    unsigned long len = strlen((const char *)ch);
    for(; i < len; ++i) {
      if(!isspace(ch[i]))
        break;
    }
        ...
        case 2:
          switch(num_leading_hyphens) {
            ...
            case 1:; //-YY; -YY-MM (implicit century)
              ...
              unsigned long current_year = [dateComps year];
              ...
              if(*ch == '-') {
                ++ch;
                month_or_week = read_segment_2digits(ch, &ch);
                NSLog(@"(%@) month is %ld", str, (long)month_or_week);
              } else {
                month_or_week = 1U;
              }
    ...
    if(isValidDate) {
      ...
      switch(dateSpecification) {
        ...
        case week:;
          ...
          unsigned  long prevYear = year - 1U;
          unsigned YY = prevYear % 100U;
          unsigned long C = prevYear - YY;
          unsigned G = YY + YY / 4U;
