...
- (id)initWithFrame:(CGRect)frame setHomePage:(NSString *)selectHomePageV setOrderDetail:(NSString *)selectOrderDetailV{
    ...
    if (self) {
        ...
        self.selectOrderDetailV=selectOrderDetailV;
        self.selectHomePageV=selectHomePageV;
        [self initData];
        [self initSearchBar];
        [self initTableView];
...
- (UIView*)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    if (tableView == _tableView) {
        ...
        NSLog(@"%@------%@",_selectOrderDetailV,_selectHomePageV);
        if ([_selectHomePageV isEqualToString:@"2"]) {
            if (section == 0) {
                label.text = @"当前城市";
            }else if (section == 1){
                //如果第一次使用没有最近访问的城市则赢该行
        ...
        }else if([_selectOrderDetailV isEqualToString:@"1"])
        {
            if (section == 0) {
                label.text = @"当前站点";
            }else if (section == 1){
...
//    _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 44)];
    ...
    {
        _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 44)];
        _searchBar.placeholder = @"输入站点名或拼音";
    }else{
        _searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 44)];
